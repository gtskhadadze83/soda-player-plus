...
export const getPaths = async (localAppdataDir) => {
    const electronAppBase = path.join(process.env.LOCALAPPDATA || "", localAppdataDir);
    if (!fse.existsSync(electronAppBase))
...
export const isPatchAvailable = async ({ localAppdataDir }) => {
    try {
...
export const patchElectronApp = async ({ localAppdataDir, appName, patchContents }) => {
    ...
    asar.extractAll(asarSource, asarUnpacked);
